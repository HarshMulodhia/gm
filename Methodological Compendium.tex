...
chunking. \cite{lecnotes2026,diffpolicy-theory-notes,gm-companion-notes,gm2025,diffusion-policy2023,act-project-page}
...
framework. \cite{act-openarm-notes,isaaclab-github} - \textbf{Algorithm Breakdown (Core):}
...
equations. \cite{diffpolicy-theory-notes,gm-companion-notes,gm2025,diffusion-policy2023,act-hf-summary,lecnotes2026}
...
token-based components. \cite{gm-companion-notes,gm2025,lecnotes2026}
...
state as
\(x^{\text{prop}}_t = (q_t, \dot q_t, g_t)\). \cite{isaaclab-github,act-openarm-notes}
...
\(T_p\). \cite{diffusion-policy2023,diffpolicy-theory-notes}
...
Prismatic-7B). \cite{openvla-notes,act-openarm-notes,isaaclab-github}
...
sequences. \cite{act-project-page,diffpolicy-theory-notes,diffusion-policy2023}
...
development environment is \cite{act-openarm-notes,isaaclab-github}
...
simulation. \cite{isaaclab-github,act-openarm-notes}
...
sequences. \cite{diffusion-policy-repo,act-openarm-notes}
...
in action space, as detailed later. \cite{diffpolicy-theory-notes,openvla-notes}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{3. Generator Matching Theory and Markov
Superpositions}\label{generator-matching-theory-and-markov-superpositions}

\textbf{Reading flow remark.} This section introduces the mathematical backbone used by all later policy and architecture sections.
...
that \cite{gm2025,gm-companion-notes} \[
 p_0(\mathrm{d}x \mid z) = p_{\text{simple}}(\mathrm{d}x), \quad p_1(\mathrm{d}x \mid z) = \delta_z, 
\] for each data point \(z \in S\). \cite{gm-companion-notes,gm2025}
...
generative modeling. \cite{lecnotes2026,gm-companion-notes}

Common path constructions include: \cite{gm2025,gm-companion-notes} -
...
\(C\). \cite{diffusion-policy2023,diffpolicy-theory-notes}
...
\(k_{t+h\mid t}(\cdot \mid x)\) satisfying \cite{gm2025,gm-companion-notes} \[
...
\]\cite{gm-companion-notes,gm2025}
...
notes. \cite{lecnotes2026,gm-companion-notes,gm2025}
...
are: \cite{lecnotes2026,gm-companion-notes,gm2025}
...
the lecture-notes taxonomy. \cite{gm2025,lecnotes2026}
...
practical training objective \cite{gm-companion-notes,gm2025} \[
...
(e.g., DDPM noise prediction loss). \cite{diffpolicy-theory-notes,gm2025}
...
sub-policies or VLA
tokens. \cite{act-project-page,diffusion-policy2023,diffpolicy-theory-notes,openvla-notes,act-openarm-notes}
...
\(S_A\). \cite{diffusion-policy2023,diffpolicy-theory-notes}

Other relevant applications of GM to VLA / OpenArm
include: \cite{gm-companion-notes,gm2025} - \textbf{Multimodal fusion:} combining
...
Policy}\label{diffusion-policy-diffusion-as-an-action-policy}

\textbf{Reading flow remark.} With the GM framework established, we next instantiate it through diffusion-based action generation.
...
\textbf{action space}. Let \cite{diffpolicy-theory-notes,diffusion-policy2023} \[
 S = \mathbb{R}^{d_A T_p}
...
distribution \cite{diffusion-policy2023,diffpolicy-theory-notes} \[
...
\(\epsilon_\theta\). \cite{diffpolicy-theory-notes,diffusion-policy2023}
...
\(k\), and draws \cite{lecnotes2026,diffpolicy-theory-notes} \[
 A_k = \alpha_k A_0 + \sigma_k \epsilon, \quad \epsilon \sim \mathcal{N}(0, I),
\] where \(\alpha_k, \sigma_k\) follow a DDPM
schedule. \cite{diffusion-policy-repo,diffpolicy-theory-notes}
...
\(\nabla_A \log p_\tau(A \mid O_t)\). \cite{diffusion-policy2023,diffpolicy-theory-notes,gm-companion-notes,lecnotes2026}
...
equivalent to conditional score matching: \cite{diffpolicy-theory-notes,diffusion-policy2023} \[
...
language, this is a special case of CGM with: \cite{diffusion-policy-repo,diffpolicy-theory-notes} -
...
Euclidean distance. \cite{gm-companion-notes,gm2025}
...
directly with the GM framework. \cite{diffpolicy-theory-notes,gm-companion-notes,gm2025}
...
architectures: \cite{diffusion-policy-repo,diffusion-policy2023} - \textbf{Diffusion Policy CNN:}
...
smooth multi-step trajectories per denoising step. \cite{diffusion-policy2023,diffpolicy-theory-notes}
...
control. \cite{diffusion-policy2023,diffpolicy-theory-notes} - \textbf{Conditioning:} Stack latest
...
model. \cite{openvla-notes,diffpolicy-theory-notes}
...
operates as follows: \cite{act-openarm-notes,diffusion-policy-repo} 1. At each decision step,
...
language context. \cite{gm-companion-notes,diffpolicy-theory-notes,gm2025}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{5. ACT: Action Chunking with
Transformers}\label{act-action-chunking-with-transformers}

\textbf{Reading flow remark.} This section complements diffusion-based control with chunked transformer policies and explains how ACT fits the same generator view.
...
\(H\). \cite{act2023,act-hf-summary,act-project-page,act-openarm-notes}

ACT is formulated as a \textbf{conditional variational autoencoder
(CVAE)} over action chunks conditioned on
observations: \cite{act-hf-summary,act2023} - \textbf{Latent variable:}
...
hyperparameter. \cite{act2023,act-hf-summary}
...
and style. \cite{act-hf-summary,act2023}
...
components: \cite{act2023,act-project-page,act-hf-summary} - \textbf{Vision
...
this includes both arms' joint states. \cite{act-project-page,act2023} -
...
actions. \cite{act-hf-summary,act-project-page}
...
dependencies. \cite{act2023,act-hf-summary}
...
generator}: \cite{act2023,gm-companion-notes,gm2025} - The latent
...
Policy. \cite{act-project-page,act2023,gm-companion-notes,gm2025}
...
per episode). \cite{act-project-page,act-openarm-notes} 3. \textbf{Training:} Use ACT's
...
\(L\), and observation modalities. \cite{act-hf-summary,act-openarm-notes} 4.
...
robustness and multimodality. \cite{diffpolicy-theory-notes,act-openarm-notes,gm2025}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{6. VLA Architecture for OpenArm (Inspired by
OpenVLA)}\label{vla-architecture-for-openarm-inspired-by-openvla}

\textbf{Reading flow remark.} After individual policy components, we now describe a unified VLA architecture that conditions and combines them.
...
information. \cite{isaaclab-github,openvla-notes} - \textbf{Projector:} A 2-layer MLP
...
ACT. \cite{diffpolicy-theory-notes,act-openarm-notes,openvla-notes}
...
over tokens if desired. \cite{openvla-notes,gm2025}
...
decomposition. \cite{diffusion-policy2023,diffpolicy-theory-notes} - \textbf{ACT head:} Produces
latent \(z\) and action chunks via a Transformer decoder, implementing
\(L_t^{\text{ACT}}\) (flow + jump-like
behavior). \cite{act2023,act-project-page,act-openarm-notes} - \textbf{Flow head
...
CTMC generator on a finite mode set. \cite{lecnotes2026,gm2025}
...
network. \cite{gm2025,openvla-notes}
...
path. \cite{act-openarm-notes,diffpolicy-theory-notes,gm2025,openvla-notes}
...
Anchor)}\label{synthesis-matrix-the-visual-anchor}

\textbf{Reading flow remark.} Use this matrix as a compact cross-reference before moving to the implementation plan.
...
OpenArm. \cite{act2023,lecnotes2026,gm-companion-notes,diffusion-policy2023,diffpolicy-theory-notes,act-openarm-notes,gm2025,openvla-notes}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{8. Execution Pipeline (The Action
Plan)}\label{execution-pipeline-the-action-plan}

\textbf{Reading flow remark.} This section converts the theory and architecture into an ordered implementation roadmap.
...
\item
  ...
  available. \cite{isaaclab-github,act-openarm-notes}
\item
  ...
  Lab. \cite{isaaclab-github,act-openarm-notes}
\item
  ...
  architectures. \cite{act-openarm-notes,openvla-notes}
\item
  ...
  Policy. \cite{diffusion-policy-repo,act-openarm-notes}
\end{enumerate}
...
\(A_0\) for Diffusion Policy. \cite{diffusion-policy-repo,diffpolicy-theory-notes,act-openarm-notes} -
Configure a diffusion model (CNN or time-series Transformer) with
horizon \(T_p\) and noise schedule. \cite{diffusion-policy-repo,diffusion-policy2023} - Train with
the noise-prediction objective \(\mathcal{L}_{\text{DP}}\), monitoring
validation success in Isaac Lab simulations. \cite{diffusion-policy2023,diffpolicy-theory-notes}

\textbf{8.2.2 Train ACT on OpenArm} - Use the OpenArm ACT adapter to
present data in ACT's expected format. \cite{act-project-page,act-openarm-notes} - Configure
chunk length \(H\), history length \(L\), and latent dimension \(d_z\)
and train with the CVAE loss
\(\mathcal{L}_{\text{ACT}}\). \cite{act-hf-summary,act2023} - Evaluate ACT in
...
corresponding to \(L_t^{\text{flow}}\). \cite{lecnotes2026,gm-companion-notes} - If using
action tokenization or discrete modes, train a CTMC model on token
sequences, estimating rate matrices \(Q_t\) or transition
probabilities. \cite{lecnotes2026,gm2025,openvla-notes}
...
networks. \cite{diffpolicy-theory-notes,act-openarm-notes,openvla-notes} - Retrain or fine-tune
...
\(L_t^{\text{ACT}}(\cdot \mid h_t)\). \cite{diffpolicy-theory-notes,act-openarm-notes,openvla-notes}
...
losses. \cite{gm2025,openvla-notes}
...
timestep. \cite{act-openarm-notes,diffpolicy-theory-notes,gm2025}
...
\item
  ...
  controllers. \cite{diffpolicy-theory-notes,act-openarm-notes,openvla-notes}
\item
  ...
  components. \cite{gm2025,openvla-notes}
\item
  ...
  observation and action normalization as needed. \cite{isaaclab-github,act-openarm-notes}
\item
  ...
  probability paths if enough data is available. \cite{gm2025,openvla-notes}
\end{enumerate}
...
visual and language
context. \cite{gm-companion-notes,act2023,lecnotes2026,diffusion-policy2023,act-openarm-notes,diffpolicy-theory-notes,gm2025,openvla-notes}
...
generators. \cite{isaaclab-github,act-openarm-notes,gm2025}
...
\bibitem{arxiv2506-layout} Anonymous. \textit{arXiv:2506.02070}. arXiv preprint, 2025. \url{https://arxiv.org/pdf/2506.02070}.
...
\item
  ...
  - \textbf{page-1}
  MITClass6.S184:GenerativeAIWithStochasticDiﬀerentialEquations,2026AnIntroductiontoFlowMat\ldots{}
\item
...
\begin{itemize}
  \item \textbf{arXiv:2506.02070} (layout target): \url{https://arxiv.org/pdf/2506.02070}. \cite{arxiv2506-layout}
