...
framework.{[}\^{}7{]}{[}\^{}8{]} - \textbf{Algorithm Breakdown (Core):}
...
state as
\(x^{\text{prop}}_t = (q_t, \dot q_t, g_t)\).{[}\^{}8{]}{[}\^{}7{]}
...
OpenVLA
(\(\Delta x,\Delta y,\Delta z,\Delta \text{roll},\Delta \text{pitch},\Delta \text{yaw},\Delta g\)).{[}\^{}10{]}
...
development environment is{[}\^{}7{]}{[}\^{}8{]}
...
in action space, as detailed later.{[}\^{}2{]}{[}\^{}10{]}
...
\(\mathcal{N}(\alpha_t z, \sigma_t^2 I)\).{[}\^{}3{]}
...
\(k_{t+h\mid t}(\cdot \mid x)\) satisfying{[}\^{}4{]}{[}\^{}3{]} \[
...
the lecture-notes taxonomy.{[}\^{}4{]}{[}\^{}1{]}
...
practical training objective{[}\^{}3{]}{[}\^{}4{]} \[
...
(e.g., DDPM noise prediction loss).{[}\^{}2{]}{[}\^{}4{]}
...
discrete latent modes or tokenized actions.{[}\^{}4{]}
...
include:{[}\^{}3{]}{[}\^{}4{]} - \textbf{Multimodal fusion:} combining
...
equivalent to conditional score matching:{[}\^{}2{]}{[}\^{}5{]} \[
...
language, this is a special case of CGM with:{[}\^{}11{]}{[}\^{}2{]} -
...
directly with the GM framework.{[}\^{}2{]}{[}\^{}3{]}{[}\^{}4{]}
...
architectures:{[}\^{}11{]}{[}\^{}5{]} - \textbf{Diffusion Policy CNN:}
...
smooth multi-step trajectories per denoising step.{[}\^{}5{]}{[}\^{}2{]}
...
control.{[}\^{}5{]}{[}\^{}2{]} - \textbf{Conditioning:} Stack latest
...
operates as follows:{[}\^{}7{]}{[}\^{}11{]} 1. At each decision step,
...
observations:{[}\^{}9{]}{[}\^{}12{]} - \textbf{Latent variable:}
...
components:{[}\^{}12{]}{[}\^{}6{]}{[}\^{}9{]} - \textbf{Vision
...
this includes both arms' joint states.{[}\^{}6{]}{[}\^{}12{]} -
...
encoder outputs and latent \(z\).{[}\^{}6{]}
...
a policy in OpenArm Isaac Lab:{[}\^{}7{]} 1. \textbf{Data collection:}
...
HDF5 dataset.{[}\^{}7{]} 2. \textbf{Dataset adapter:} Write an
...
per episode).{[}\^{}6{]}{[}\^{}7{]} 3. \textbf{Training:} Use ACT's
...
\(L\), and observation modalities.{[}\^{}9{]}{[}\^{}7{]} 4.
...
robustness and multimodality.{[}\^{}2{]}{[}\^{}7{]}{[}\^{}4{]}
...
structure:{[}\^{}10{]} - \textbf{Vision encoder:} Two ViT-style encoders
...
information.{[}\^{}8{]}{[}\^{}10{]} - \textbf{Projector:} A 2-layer MLP
projects visual embeddings into the language model embedding space,
implementing a ``patch-as-token'' design.{[}\^{}10{]} - \textbf{Language
...
pooled state) to feed into the action generator.{[}\^{}10{]}
...
possible:{[}\^{}10{]} 1. \textbf{Tokenized actions:} Follow OpenVLA
...
conditions all downstream action generators:{[}\^{}10{]} -
...
CTMC generator on a finite mode set.{[}\^{}1{]}{[}\^{}4{]}
...
pipeline:{[}\^{}10{]} 1. \textbf{Backbone pretraining:} Train the
vision--language backbone on large-scale robot datasets (e.g., Open
X-Embodiment) or reuse an OpenVLA-style pretrained model.{[}\^{}10{]} 2.
...
LLM-based language backbone.{[}\^{}10{]} - Ensure the backbone can
ingest OpenArm images and instructions, producing representations
\(h_t\) that correlate with task-relevant visual features.{[}\^{}10{]}
...
networks.{[}\^{}2{]}{[}\^{}7{]}{[}\^{}10{]} - Retrain or fine-tune
...
\(L_t^{\text{ACT}}(\cdot \mid h_t)\).{[}\^{}2{]}{[}\^{}7{]}{[}\^{}10{]}
...
\item
  ...
  observation and action normalization as needed.{[}\^{}8{]}{[}\^{}7{]}
\item
  ...
  probability paths if enough data is available.{[}\^{}4{]}{[}\^{}10{]}
\end{enumerate}
...
\subsection{Overvi\ldots{}}\label{overvi}
...
\subsection{1. Goal and Hi\ldots{}}\label{goal-and-hi}
...
\section{Submission-Ready Addendum: Algorithms, Theorems, and Innovations}
...
This section adds formal GM constructions that are directly reusable for OpenArm VLA policies when one needs richer training targets and more expressive sampling dynamics.{[}\^{}4{]}
...
\]
where \(u_t\) is the flow field, \(\Sigma_t \succeq 0\) is the diffusion covariance, and \(Q_t\) is a jump measure.{[}\^{}4{]}
...
\]
These equations are useful for implementing posterior-averaged OpenArm action generators conditioned on observation-language context.{[}\^{}4{]}
...
\]
This justifies training OpenArm models with conditional supervision while still optimizing the intended marginal process.{[}\^{}4{]}

\subsubsection{D. Constructive Jump and Diffusion Examples}

The paper provides explicit constructive solutions that demonstrate how non-standard generators can still realize prescribed paths:{[}\^{}4{]}
\begin{itemize}
...
\(\langle p_t,L_t^{\mathrm{div}}f\rangle=0\), yielding predictor--corrector families that preserve marginals while changing trajectories.{[}\^{}4{]}
